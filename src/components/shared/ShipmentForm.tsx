...
export default function ShipmentForm() {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      const { country_code, postal_code } = shipment.ship_to;
      const shipFrom = shipment.ship_from;

      if (postal_code !== "20500") {
        toast.error("shipment to postal code should be '20500'");
        return null;
      }
      if (shipFrom.postal_code !== "78731") {
        toast.error("shipment from postal code should be '78731'");
        return null;
      }
      if (country_code !== "US" || shipFrom.country_code !== "US") {
        toast.error("country code should be 'US'");
        return null;
      }
      const { packages } = shipment;
      if (packages[0].package_code !== "package") {
        toast.error("package code should be 'package'");
        return null;
      }
